...
def registration():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        try:
            ...
            user = UserProfile(username=username, password=password, first_name=firstname, last_name=lastname, email=email, location=location, biography=biography, photograph=photograph, date_joined=user_date)
            photograph.save(os.path.join(app.config['PROFILE_PIC'], filename))
            db.session.add(user)
            db.session.commit()
            
            rjsonify(message = "User successfully registered") 
